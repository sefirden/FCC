...
public class Calculations : MonoBehaviour
{
    ...
    private void Awake() //запускаетс€ до всех стартов
    {
    ...
    public void SetSliderInput()
    {
        ...
        if (ui.slidersForInput.Count < Settings.Instance.sliderNumbersQ + Settings.Instance.decimalSlider)
        {
            for (int i = 0; i < (Settings.Instance.sliderNumbersQ + Settings.Instance.decimalSlider); i++)
            {
                ...
                slider_temp.name = "slider_temp"; //присваиваем им€
    ...
    public IEnumerator SetSliderDecimal()
    {
        ...
        if (Settings.Instance.sliderNumbersQ < 5 && Settings.Instance.decimalSlider > 0)
        {
            ...
            decimal_temp.name = "decimal_temp"; //присваиваем им€
    ...
    public void OnInputChange() //дл€ инпут пол€
    {        
        try
        {
    ...
    {
        double value;
        //при расчетах провер€ем если число больше 62 то делим его на ввоодимое значение, если меньше то множим
        if (staticvalue > 62)
        {
...
public class FormulaList
{
    ...
    public FormulaList(string F, string T, double Fr)//в таком пор€дке заполн€ем данные дл€ пиццы
    {
        ConvertFrom = F;
        ConvertTo = T;
        Formula = Fr;
